...
class PacketSender {
  ...
  static sendGameStart(game) {
    const pieces = Piece.generatePieces(0);
  ...
  /**
   * Send a set of pieces to a player.
   * @param {Player} player
   * @param {Game} game
   * @param {Array<number>} pieces
   */
  static sendGenFlowToPlayer(player, game, pieces) {
    PacketSender.sendPacketToPlayer(socketDefs.PACKET_GENFLOW, player, game, {pieces});
  }
   ...
   * @param  data
   */
  static sendPacketToPlayer(packetName, player, game, data) {
    if (!game) {
      return;
    }
    console.log(player);
    const socket = SocketMap.sockets.get(player);
    if (socket)
      socket.emit(packetName, data);
  }
}
