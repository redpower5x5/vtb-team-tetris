...
class TetrisSocketHandler extends SocketHandler {
  ...
  /**
   * Send to player pieces.
   * @param {String} data
   * @param {String} data.roomName
   * @param response
   */
  genPlayerFlow(data, response = socketDefs.GENPLAYERFLOW_RESPONSE) {
    if (this.gameIsValid(data, response))
    {
      const tetrisPieces = Piece.generatePieces(10);
      this.socket.emit(response, {success: true});
      PacketSender.sendGenFlowToPlayer(data.player_id, GameManager.getGame(data.roomName), tetrisPieces);
    }
  }
